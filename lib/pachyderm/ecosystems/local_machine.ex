defmodule Pachyderm.Ecosystems.LocalMachine do
  def participate(ref \\ make_ref()) do
    ...
    p1 =
      Supervisor.start_child(Pachyderm.Ecosystem.LocalMachine.Supervisor, worker_sup_spec)
      |> case do
        {:ok, pid} -> pid
    ...
    p2 =
      Supervisor.start_child(Pachyderm.Ecosystem.LocalMachine.Supervisor, task_sup_spec)
      |> case do
        {:ok, pid} -> pid
        {:error, {:already_started, pid}} -> pid
      end
